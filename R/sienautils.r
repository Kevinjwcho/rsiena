                           ...
                           imputationValues = NULL)
{
  ...
  if (!sparse)
  {
    # Non-sparse case: must be an array
    ...
    if (length(netdims) == 4)
    {
      ...
      else
      {
        ...
        if (type != "threeway")
        {
          stop("incorrect type for 4D array; use type='threeway'")
        }
      }
    ...
    {
      # Original behavior: if 2D, treat as behavior (reshape to N×1×T)
      ...
      # Determine network type (behavior/continuous/bipartite/oneMode) for 3D arrays
      if (netdims[2] == 1) ## how behavior array was defined earlier
      {
        if (missing(type))
        {
          type <- "behavior"
  ...
  {
    # Sparse case: must be a list of TsparseMatrix with identical dimensions
    # require(Matrix)
    if (!is.list(netarray))
    {
      stop("netarray must be an array or a list of sparse matrices")
    ...
    # NOTE: sparse threeway is not supported in this minimal extension.
    # You can relax this if you implement 4D sparse handling (list-of-3D or similar).
    if (missing(type))
    {
      # keep original logic for sparse (no auto threeway here)
      if (netdims[2] == 1)
      {
        type <- "behavior"
      }
    ...
    else
    {
      ...
      if (type == "threeway")
      {
        stop("sparse threeway not implemented in this constructor")
      }
    }
  }
  
  # Warn if everything is missing
  ...
  # warning for 
  if (!identical(type, "threeway") && isTRUE(shareParameters)) {
  ...
  # - oneMode/bipartite: N×N×T
  # - threeway: N×N×N×T (checked above)
  observations <- if (length(netdims) == 4) netdims[4] else netdims[3]
  if (observations < 2)
  {
    stop("netarray must have at least two observations")
  }
  
  # Validate nodeSet based on type:
  if (type == "bipartite")
  {
    # Bipartite needs exactly 2 node sets
  ...
  {
    # oneMode / behavior / continuous: exactly one node set (character)
    if (is.vector(nodeSet) && length(nodeSet) > 1)
    {
  ...
  # Value checks for non-behavior, non-continuous:
  # Allow 0, 1, 10, 11, or NA. (10=structural zero, 11=structural one)
  # This should also cover the threeway type since it is neither behavior nor continuous.
  if (type != "behavior" && type != "continuous")
  {
    if (sparse)
    {
      ...
      {
        tmp <- x@x
        all(is.na(tmp) | tmp == 1 | tmp == 10 | tmp == 11 )
      })))
      {
    ...
    {
      # For arrays (including 4D threeway), check entries directly
  ...
  # imputationValues are only implemented for dependent behavior variables
  # Keep this original behavior: do not allow for threeway/oneMode/bipartite
  if (!is.null(imputationValues))
  {
    if (type != "behavior" && type != "continuous")
    {
    ...
    if (length(netdims) == 4)
    {
      stop("imputationValues are not supported for type='threeway'")
    }
  ...
  # Build and return the sienaDependent object
  obj <- netarray
