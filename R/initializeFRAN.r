                           ...
                           returnLoglik=FALSE, onlyLoglik=FALSE)
{
  ...
  if (!initC) ## i.e. first time round
  {
    ...
    ## --- BEGIN: threeway-only bookkeeping fix ---
    ## If the dependent variable is threeway AND effects are already slice-expanded
    ## (e.g., "Y[1]"..."Y[self]"), then use those slice names as the internal
    ## dependent-variable names for initializeFRAN bookkeeping.
    depv_check <- data[[1]]$depvars
    threeway_flag <- any(vapply(depv_check,
                                function(v) identical(attr(v, "type"), "threeway"),
                                logical(1)))
    
    ## requestedEffects exists above (effects[effects$include, ])
    slice_expanded_effects <- any(grepl("\\[[^\\]]+\\]$", requestedEffects$name))
    
    if (threeway_flag && slice_expanded_effects) {
      slice_netnames <- unique(requestedEffects$name)
      slice_netnames <- slice_netnames[slice_netnames != "sde" & nzchar(slice_netnames)]
      
      ## Make initializeFRAN's depvar bookkeeping consistent with effects$name
      depvarnames <- slice_netnames
      
      ## IMPORTANT: later code sets attr(f,"netnames") <- attr(data,"netnames"),
      ## and then checks attr(f,"netnames") %in% effects$name.
      ## So we must update data-level netnames here.
      attr(data, "netnames") <- slice_netnames
      
      ## Also set netnames on each group element (safe for downstream code paths)
      for (g in seq_along(data)) {
        attr(data[[g]], "netnames") <- slice_netnames
      }
    }
    ## --- END: threeway-only bookkeeping fix ---
    
    effects1 <- split(requestedEffects, requestedEffects$name)
    effects1order <- match(c(depvarnames, "sde"), names(effects1))
    
    ## If match() fails (e.g., depvarnames does not align with split names),
    ## do NOT let do.call(rbind, NULL) return NULL.
    ## Apply a threeway-only fallback order using names present in requestedEffects.
    if (threeway_flag && (length(effects1order) == 0 || all(is.na(effects1order)))) {
      depvarnames_eff <- unique(requestedEffects$name)
      depvarnames_eff <- depvarnames_eff[depvarnames_eff != "sde" & nzchar(depvarnames_eff)]
      effects1order <- match(c(depvarnames_eff, "sde"), names(effects1))
    }
    
    ## Drop NA indices before rbind
    effects1order <- effects1order[!is.na(effects1order)]
    requestedEffects <- do.call(rbind, effects1[effects1order])
    
    ## If still NULL (should not happen), stop with a clear message
    if (is.null(requestedEffects)) {
    ...
    if (threeway_flag && (length(effects1order) == 0 || all(is.na(effects1order)))) {
      depvarnames_eff <- unique(effects$name)
      depvarnames_eff <- depvarnames_eff[depvarnames_eff != "sde" & nzchar(depvarnames_eff)]
      effects1order <- match(c(depvarnames_eff, "sde"), names(effects1))
    }
    ...
    if (is.null(effects)) {
      stop("Internal error: effects became NULL after reordering. Check effects/name matching.")
    }
    ...
    if (is.na(x$cconditional))
    {
      ...
      x$cconditional <- (!threeway_flag) && (!slice_expanded_effects) &&
        (!x$maxlike) && (length(depvarnames) == 1)
      if (x$cconditional)
      {
        x$condvarno <- 1
      }
    ...
    ## --- threeway-only: override netnames using unpacked nets names ---
    depv_check <- data[[1]]$depvars
    threeway_flag <- any(vapply(depv_check,
                                function(v) identical(attr(v, "type"), "threeway"),
                                logical(1)))
  ...
  if (!initC)
  {
    ...
    {
      cat("CHECK netnames attr(f):\n"); print(attr(f, "netnames"))
      cat("CHECK nets names f[[1]]$nets:\n"); print(names(f[[1]]$nets))
      cat("CHECK effects names:\n"); print(unique(effects$name))
...
unpackData <- function(data, x)
{
  ...
  ## ------------------------------------------------------------
  ## Helper: compute distance vector (length = T-1) for a oneMode array (n x n x T)
  ## distance[t] = number of dyads whose value changed between t and t+1 (ignoring NAs)
  ## ------------------------------------------------------------
  .computeDistance_oneMode <- function(arr3) {
    dd <- dim(arr3)
    if (length(dd) != 3) stop("Expected 3D array (n x n x T) for oneMode distance.")
    TT <- dd[3]
    if (TT < 2) return(numeric(0))
    ...
    for (tt in 1:(TT - 1)) {
      ...
      a2 <- arr3[, , tt + 1]
      mydiff <- a2 - a1
      dist[tt] <- sum(mydiff != 0, na.rm = TRUE)
    }
    dist
  }
  
  ## ------------------------------------------------------------
  ## Helper: expand a threeway depvar into a named list of oneMode sienaDependent objects
  ## Output names: baseName[1]...baseName[K], baseName[self]
  ##
  ## Assumptions:
  ## - attr(depvar,"netdims") == c(K, n, n, T)
  ## - depvar[k, i, j, t] exists
  ## - attr(depvar,"symmetric") may be scalar or length-2 (perc, self)
  ## ------------------------------------------------------------
  .expandThreewayToOneModes <- function(depvar, baseName, observations) {
    netdims <- attr(depvar, "netdims")
    if (length(netdims) != 4) stop("threeway depvar must have 4D netdims (K, n, n, T).")
    ...
    if (TT != observations) {
      ## Keep going, but warn because downstream expects observations to match
      warning("threeway netdims[4] (T) does not match data$observations.")
    }
    
    ## symmetric handling: prefer length-2 c(sym_perception, sym_self), otherwise default FALSE
    sym <- attr(depvar, "symmetric")
    if (is.null(sym)) sym <- FALSE
    if (length(sym) == 1) sym <- c(sym, FALSE)
    if (length(sym) != 2) sym <- c(FALSE, FALSE)
    
    ao <- attr(depvar, "allowOnly")
    if (is.null(ao) || length(ao) == 0) ao <- FALSE
    ao <- as.logical(ao)[1]
    
    ## Optional precomputed distances (recommended if you stored them in sienaDataCreate)
    distSlices <- attr(depvar, "distanceSlices")  # expected list length K, each numeric length T-1
    distSelf   <- attr(depvar, "distanceSelf")    # expected numeric length T-1
    
    out <- vector("list", K + 1)
    
    ## (A) perception slices: k = 1..K
    for (k in 1:K) {
      ...
      attr(arr3, "netdims") <- c(n1, n2, TT)
      attr(arr3, "nodeSet") <- attr(depvar, "nodeSet")
      attr(arr3, "name") <- paste0(baseName, "[", k, "]")
      attr(arr3, "symmetric") <- sym[[1]]
      ## Required numeric attributes expected later by unpackOneMode/C code
      attr(arr3, "balmean") <- rep(0, TT)
      attr(arr3, "structmean") <- rep(0, TT)
      attr(arr3, "averageInDegree") <- rep(0, TT)
      attr(arr3, "averageOutDegree") <- rep(0, TT)
      attr(arr3, "settingsinfo") <- list()
      attr(arr3, "sparse") <- FALSE
      ## Up/down only flags (placeholders)
      attr(arr3, "uponly") <- rep(FALSE, TT - 1)
      attr(arr3, "downonly") <- rep(FALSE, TT - 1)
      attr(arr3, "allowOnly") <- ao
    ...
    ## (B) self slice: self[i, j, t] = depvar[slice=i, sender=i, receiver=j, t]
    self_arr <- array(NA_real_, dim = c(n1, n2, TT))
    for (tt in 1:TT) {
      for (i in 1:n1) {
        self_arr[i, , tt] <- depvar[i, i, , tt]
      }
    }
    
    class(self_arr) <- "sienaDependent"
    attr(self_arr, "type") <- "oneMode"
    attr(self_arr, "netdims") <- c(n1, n2, TT)
    attr(self_arr, "nodeSet") <- attr(depvar, "nodeSet")
    attr(self_arr, "name") <- paste0(baseName, "[self]")
    attr(self_arr, "symmetric") <- sym[[2]]
    attr(self_arr, "balmean") <- rep(0, TT)
    attr(self_arr, "structmean") <- rep(0, TT)
    attr(self_arr, "averageInDegree") <- rep(0, TT)
    attr(self_arr, "averageOutDegree") <- rep(0, TT)
    attr(self_arr, "settingsinfo") <- list()
    attr(self_arr, "sparse") <- FALSE
    attr(self_arr, "uponly") <- rep(FALSE, TT - 1)
    attr(self_arr, "downonly") <- rep(FALSE, TT - 1)
    attr(self_arr, "allowOnly") <- ao
    attr(self_arr, "allUpOnly") <- FALSE
    attr(self_arr, "allDownOnly") <- FALSE
    
    if (!is.null(distSelf)) {
      attr(self_arr, "distance") <- distSelf
    } else {
      attr(self_arr, "distance") <- .computeDistance_oneMode(self_arr)
    }
    
    out[[K + 1]] <- self_arr
    
    names(out) <- c(paste0(baseName, "[", 1:K, "]"), paste0(baseName, "[self]"))
    out
  }
  ...
  threeways <- data$depvars[types == "threeway"]
  
  ## ------------------------------------------------------------
  ## NEW: expand threeway depvars into multiple oneMode depvars and append to oneModes
  ## ------------------------------------------------------------
  if (length(threeways) > 0) {
    for (j in seq_along(threeways)) {
      baseName <- names(threeways)[j]
      expanded <- .expandThreewayToOneModes(threeways[[j]], baseName, observations)
      oneModes <- c(oneModes, expanded)
    }
  }
  ...
  ## Build f object
  ## ------------------------------------------------------------
  f$nDepvars <- length(oneModes) + length(Behaviors) + length(continuousBehaviors) + length(bipartites)
  ...
  ## Keep original depvars (including threeway) for reference, but model nets come from oneModes
  ...
  ## Up/down only flags are defined for oneModes/behaviors
  ...
  f$behaviorDownOnly <- sapply(Behaviors, function(x) attr(x, "downonly"))
  
  ## Distances: compute from the oneMode objects actually used in f$nets
  f$distances <- sapply(oneModes, function(x) attr(x, "distance"))
